// Ported near-verbatim from apps/web/src/features/vocab/vocabSlice.ts —
// plain Redux Toolkit + axios calls against src/lib/api.ts's instance,
...
export const searchVocab = createAsyncThunk(
  ...
  async ({ word, miniAppId }: { word: string; miniAppId: string }, { rejectWithValue }) => {
    try {
      const res = await api.get('/vocab/search', { params: { word, miniAppId } });
...
export const fetchTermDetail = createAsyncThunk(
  ...
  async (termId: string, { rejectWithValue }) => {
    try {
      const res = await api.get(`/vocab/terms/${termId}`);
...
export const addDefinitionToBucket = createAsyncThunk(
  ...
  async (
    ...
    try {
      await api.post('/vocab/bucket', { termId, definitionId, miniAppId });
      return { definitionId };
    } catch (err: unknown) {
      const e = err as { response?: { status?: number } };
...
export const fetchTrending = createAsyncThunk(
  ...
  async ({ miniAppId, limit = 10 }: { miniAppId: string; limit?: number }, { rejectWithValue }) => {
    try {
      const res = await api.get('/vocab/trending', { params: { miniAppId, limit } });
...
export const fetchRecent = createAsyncThunk(
  ...
  async ({ miniAppId, limit = 10 }: { miniAppId: string; limit?: number }, { rejectWithValue }) => {
    try {
      const res = await api.get('/vocab/recent', { params: { miniAppId, limit } });
...
export const fetchAlphabet = createAsyncThunk(
  ...
  async (miniAppId: string, { rejectWithValue }) => {
    try {
      const res = await api.get('/vocab/dictionary/alphabet', { params: { miniAppId } });
...
export const browseDictionary = createAsyncThunk(
  ...
  async (
    ...
    try {
      const res = await api.get('/vocab/dictionary', {
...
export const fetchBucket = createAsyncThunk(
  ...
  async (
    ...
    try {
      const res = await api.get('/vocab/bucket', {
...
export const removeBucketEntry = createAsyncThunk(
  ...
  async ({ termId, miniAppId }: { termId: string; miniAppId: string }, { rejectWithValue }) => {
    try {
      await api.delete(`/vocab/bucket/${termId}`, { params: { miniAppId } });
      return { termId };
    } catch (err) {
      return rejectWithValue({
        termId,
